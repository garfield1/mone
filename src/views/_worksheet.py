...
def state_transfer(user,action,w,reject_reason=None):
	...
	if action == WS_USER_ACTION_DEVELOPER_CREATED or action == WS_USER_ACTION_DEVELOPER_RESUBMIT:
		...
		ws = WorksheetState.objects.create(creator = user, waitting_confirmer = user.organization.leader ,worksheet = w, state = WS_STATE_WAITTING_TEAM_LEADER_CONFIRMED , action = action)
